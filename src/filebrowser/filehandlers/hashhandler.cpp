...
bool HashFileHandler::packFiles(const QString &exportDirectoryPath, const QString &sourceDirectoryPath) {
    ...
    exportFile.write(QString("Hash").toUtf8());
    ...
    for (quint32 i = 0; i < fileEntries.size(); i++) {
        ...
        quint32 index = 0;
        for (quint32 j = 0; j < sortedEntries.size(); j++) {
            if (sortedEntries[j].hash == entry.hash) {
                index = j;
                break;
            }
        }
...
bool HashFileHandler::parseFile() {
    ...
    quint32 signatureOffset = entryCount * 12 + file.pos();
    file.seek(signatureOffset);
    QString signatureInFile = file.read(16).data();
    ...
    if (signatureInFile == signature_) {
        for (quint32 i = 0; i < entryCount; i++) {
            ...
            quint32 fileHash = toLittleEndian(file.read(4));
...
QByteArray HashFileHandler::readString(QFile& file) const {
    QByteArray outBytes;
    char currentChar = file.read(1)[0];
    while (currentChar != '\0') {
